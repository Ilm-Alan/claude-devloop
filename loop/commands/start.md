---
description: "Start a loop for iterative development"
...
```!
LOOP_ARGS="${ARGUMENTS}" "${CLAUDE_PLUGIN_ROOT}/scripts/setup-loop.sh"
```
...
```
/loop:start Build user registration --max-iterations 10 --completion-promise 'Registration flow complete with tests'
```
