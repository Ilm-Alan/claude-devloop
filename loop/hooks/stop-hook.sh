...
# Look for loop-*.local.md files and match by stored transcript path
STATE_FILE=""
...
if [[ ! "$ITERATION" =~ ^[0-9]+$ ]]; then
  echo "Warning: Loop state corrupted, stopping." >&2
  rm -f "$STATE_FILE"
  exit 0
fi

if [[ ! "$MAX_ITERATIONS" =~ ^-?[0-9]+$ ]]; then
  echo "Warning: Loop state corrupted (invalid max_iterations: '$MAX_ITERATIONS'), stopping." >&2
  rm -f "$STATE_FILE"
  exit 0
fi
    ...
    if grep -q '"role":"assistant"' "$TRANSCRIPT_PATH"; then
      ...
      if [[ -n "$LAST_OUTPUT" ]]; then
        ...
        if [[ -n "$PROMISE_TEXT" ]] && [[ "$PROMISE_TEXT" = "$COMPLETION_PROMISE" ]]; then
          echo "Loop: Detected <promise>$COMPLETION_PROMISE</promise>"
...
if [[ "$COMPLETION_PROMISE" != "null" ]] && [[ -n "$COMPLETION_PROMISE" ]]; then
  USER_MSG="Loop iteration $NEXT_ITERATION | To exit: output <promise>$COMPLETION_PROMISE</promise> (ONLY when statement is TRUE - do not lie to exit!)"
else
  USER_MSG="Loop iteration $NEXT_ITERATION | No completion promise set - loop runs infinitely"
fi
